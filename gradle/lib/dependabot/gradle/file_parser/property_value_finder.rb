    ...
    class FileParser
      class PropertyValueFinder
        ...
        KOTLIN_MULTI_PROPERTY_DECLARATION_REGEX =
          /\s*extra\[['"](?<namespace>[^\s=]+)['"]\]\s*=\s*listOf\((?<values>[^\]]+)\)/m.freeze
        ...
        def fetch_kotlin_block_property_declarations(buildfile)
          ...
          prepared_content(buildfile).scan(KOTLIN_BLOCK_PROPERTY_DECLARATION_REGEX) do
            captures = Regexp.last_match.named_captures
            namespace = captures.fetch("namespace")

            captures.fetch("values").scan(KOTLIN_SINGLE_PROPERTY_SET_REGEX) do
              declaration_string = Regexp.last_match.to_s.strip
              sub_captures = Regexp.last_match.named_captures
              name = sub_captures.fetch("name")
              full_name = (namespace == "extra") ? name : [namespace, name].join(".")
