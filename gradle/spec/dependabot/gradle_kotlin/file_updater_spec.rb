...
RSpec.describe Dependabot::Gradle::FileUpdater do
  ...
  describe "#updated_dependency_files" do
    ...
    describe "the updated build.gradle.kts file" do
      subject(:updated_buildfile) do
        updated_files.find { |f| f.name == "build.gradle.kts" }
      end

      its(:content) do
        is_expected.to include(
          "implementation(group = \"co.aikar\", name = \"acf-paper\", version = "\
      ...
      context "with multiple buildfiles" do
        ...
        context "when both buildfiles are affected" do
          ...
          describe "the build.gradle.kts file" do
            its(:content) do
              is_expected.to include(
                "implementation(group = \"co.aikar\", name = \"acf-paper\", version = "\
                "\"0.6.0-SNAPSHOT\", changing: true)"
          ...
          describe "the app/build.gradle file" do
            subject(:updated_buildfile) do
              updated_files.find { |f| f.name == "app/build.gradle.kts" }
            end
      ...
      context "with a dependency name defined by a property" do
        let(:buildfile_fixture_name) { "name_property.gradle.kts" }
        ...
        it "updates the version in the build.gradle.kts" do
          expect(updated_buildfile.content).
            to include("implementation(\"org.jetbrains.kotlin:$nameProp:23.6-jre\"")
        end
      end
      ...
      context "with a dependency version defined by a property" do
        ...
        let(:subproject_buildfile) do
          Dependabot::DependencyFile.new(
            name: "subproject/build.gradle.kts",
            content:
              fixture("buildfiles", "kotlin", subproject_fixture_name)
          )
        end

        let(:buildfile_fixture_name) { "basic_build.gradle.kts" }
        let(:subproject_fixture_name) { "shortform_build.gradle.kts" }
        let(:dependencies) do
          [
            Dependabot::Dependency.new(
              ...
              previous_version: "1.1.4-3",
              requirements: [{
                file: "subproject/build.gradle.kts",
                requirement: "23.6-jre",
                groups: [],
                source: {
                  type: "maven_repo",
        ...
        it "updates the version in the subproject/build.gradle.kts" do
          expect(updated_files.map(&:name)).to eq(["subproject/build.gradle.kts"])
          expect(updated_files.first.content).
            to include("extra[\"kotlinVersion\"] = \"23.6-jre\"")
        end

        context "that is inherited from the parent buildfile" do
          let(:buildfile_fixture_name) { "shortform_build.gradle.kts" }
          let(:subproject_fixture_name) { "inherited_property.gradle.kts" }
    ...
    context "when updating a script plugin" do
      ...
      let(:script_plugin) do
        Dependabot::DependencyFile.new(
          ...
          content: fixture("script_plugins", "kotlin", "dependencies.gradle.kts")
        )
      end
      let(:dependency) do
        Dependabot::Dependency.new(
      ...
      describe "the updated plugin script file" do
        subject(:updated_buildfile) do
          updated_files.find { |f| f.name == "gradle/dependencies.gradle.kts" }
        end
