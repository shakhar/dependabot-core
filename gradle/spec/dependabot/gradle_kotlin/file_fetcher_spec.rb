...
RSpec.describe Dependabot::Gradle::FileFetcher do
  ...
  context "with a basic buildfile" do
    before do
      stub_request(:get, url + "?ref=sha").
          ...
          to_return(
            status: 200,
            body: fixture("github", "kotlin", "contents_java_with_subdir.json"),
      ...
      stub_request(:get, File.join(url, "build.gradle.kts?ref=sha")).
        ...
        to_return(
          status: 200,
          body: fixture("github", "kotlin", "contents_java_basic_buildfile.json"),
          headers: { "content-type" => "application/json" }
        )
    ...
    context "with a settings.gradle.kts" do
      before do
        ...
        stub_request(:get, File.join(url, "settings.gradle.kts?ref=sha")).
          ...
          to_return(
            status: 200,
            body: fixture("github", "kotlin", "contents_java_simple_settings.json"),
            headers: { "content-type" => "application/json" }
          )
  ...
  context "with a script plugin" do
    ...
    it "fetches the buildfile and the dependencies script" do
      expect(file_fetcher_instance.files.count).to eq(2)
      expect(file_fetcher_instance.files.map(&:name)).
        to match_array(%w(build.gradle.kts gradle/dependencies.gradle))
    end
    ...
    context "that can't be found" do
      before do
        ...
        stub_request(:get, File.join(url, "gradle?ref=sha")).
          ...
          to_return(
            status: 200,
            body: fixture("github", "kotlin", "contents_with_settings.json"),
            headers: { "content-type" => "application/json" }
          )
      end
