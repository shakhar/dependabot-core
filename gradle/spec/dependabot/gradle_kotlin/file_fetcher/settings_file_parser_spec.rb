...
RSpec.describe Dependabot::Gradle::FileFetcher::SettingsFileParser do
  ...
  let(:settings_file) do
    Dependabot::DependencyFile.new(
      ...
      content: fixture("settings_files", "kotlin", fixture_name)
    )
  end
  let(:fixture_name) { "simple_settings.gradle.kts" }
  ...
  describe "#subproject_paths" do
    ...
    context "with commented out subproject declarations" do
      let(:fixture_name) { "comment_settings.gradle.kts" }
    ...
    context "with multiple subprojects" do
      let(:fixture_name) { "multi_subproject_settings.gradle.kts" }
      ...
      context "declared across multiple lines" do
        let(:fixture_name) { "multiline_settings.gradle.kts" }
    ...
    context "with custom paths specified" do
      let(:fixture_name) { "custom_dir_settings.gradle.kts" }
